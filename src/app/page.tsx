...
export default async function Home({ searchParams }: { searchParams: Promise<{ submit?: string; puzzle?: string }> }) {
  ...
  return <main><PuzzleGame puzzle={daily.board} puzzleId={daily.id} editionDate={daily.editionDate} accountName={authData.user?.email?.split("@")[0]} resumeSubmission={query.submit === "pending"} sequenceNumber={sequence} dailyLimit={DAILY_LIMIT} completedNumbers={completedNumbers} /></main>;
}
