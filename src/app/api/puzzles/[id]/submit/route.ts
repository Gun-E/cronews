...
const submissionSchema = z.object({
  ...
  elapsedSeconds: z.number().int().min(1).max(604800),
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const [profile] = userId ? await db.select().from(userProfiles).where(eq(userProfiles.userId, userId)).limit(1) : [];
  ...
  const values = {
    ...
    displayName: userId ? (profile?.nickname ?? authData.user?.user_metadata?.full_name ?? authData.user?.email?.split("@")[0] ?? userId.slice(0, 8)) : parsed.data.displayName,
    correctCount,
