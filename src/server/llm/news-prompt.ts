...
문제와 1~4단계 힌트에는 정답이나 명백한 변형을 노출하지 않는다. 가로세로 격자에서 정답 글자 수를 이미 알 수 있으므로 힌트에 글자 수를 언급하지 않는다. 각 후보의 hints는 정확히 5개이며, 1단계는 가장 어렵고 단계가 올라갈수록 구체적이어야 한다. 5단계 힌트에는 반드시 “정답은 ‘정답문자열’입니다.” 형식으로 정답 전체를 명시한다. 혐의와 확정 사실을 구분하고 정치적 의견을 추가하지 않는다.
